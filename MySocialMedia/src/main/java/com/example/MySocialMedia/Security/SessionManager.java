...
public class SessionManager {
    ...
    public List<User> activeSessions() throws SQLException {
        List<User> users = new ArrayList<>();
        for(String t:sessions.keySet()){
            if(sessions.get(t).getUserService() != null) {
                users.add(sessions.get(t).getUserService().getUserDetails());
            }
        }
        return users;
    }
    ...
    public void kickFromSession(long userId) throws SQLException {
        for(String t:sessions.keySet()){
            if(sessions.get(t).getUserService() != null) {
                if (sessions.get(t).getUserService().getUserDetails().getId() == userId) {
                    endSession(t);
                    break;
                }
            }
    ...
    public AdminService getServiceAdmin(String token) throws SQLException {
        if (checkIfKeyExists(token)) {
            return sessions.get(token).getAdminService();
        } else {
            throw new SQLException("Session is over please login again");
        }
    }
